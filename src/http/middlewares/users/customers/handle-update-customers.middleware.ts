...
/**
 * Customers can only update your own informations.
 * Administrators can update any customer
 */

const HandleUpdateCustomersMiddleware = (request: Request, response: Response, next: NextFunction) => {
    ...
    try {
        ...
        if (tokenPayload?.data.id !== id && tokenPayload?.data.roleId === roles.CUSTOMER || tokenPayload?.data.roleId === roles.ATTENDANT) {
...
export default HandleUpdateCustomersMiddleware;
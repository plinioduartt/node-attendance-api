...
/**
 * Attendants can only retrieve your own informations.
 * Administrators can retrieve any attendant
 */

const HandleRetrieveAttendantsMiddleware = (request: Request, response: Response, next: NextFunction) => {
    ...
    try {
        ...
        if (tokenPayload?.data.id !== id && tokenPayload?.data.roleId === roles.ATTENDANT || tokenPayload?.data.roleId === roles.CUSTOMER) {
...
export default HandleRetrieveAttendantsMiddleware;
...
class AttendanceController {
  ...
  private hateoas: IHateoas<AttendanceController>;

  constructor(AttendanceService: IAttendanceService, hateoas: IHateoas<AttendanceController>) {
    this._service = AttendanceService;
    
    const contextName: string = '/attendances'
    this.hateoas = hateoas
    this.hateoas.registerContext(contextName)
    this.hateoas.registerEndpoints(attendancesEndpoints)
  }
  ...
  async list(request: Request, response: Response) {
    try {
      ...
      const attendancesWithCollection: AttendanceDtoType[] =
        await this.hateoas
          .injectEachCollection<AttendanceDtoType>({
